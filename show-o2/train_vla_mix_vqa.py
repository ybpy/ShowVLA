...
def main():
    ...
    def create_distributed_dataloader(dataset, batch_size, collate_fn):
    ...
    if config.training.grounding_metas_path:
        ...
        train_dataloader_grounding = create_distributed_dataloader(dataset,
    ...
    vqa_loader = create_distributed_dataloader(vqa_dataset, config.training.batch_size_vqa, vqa_dataset.collate_fn)
    ...
    def prepare_latents_for_und(
        ...
        xt = image_latents
        t = torch.ones(b*n, dtype=image_latents.dtype, device=image_latents.device)
    ...
    for batch in mixed_loader:
        ...
        with accelerator.accumulate(model):
            ...
            text_tokens = vqa_batch['text_tokens'].to(accelerator.device)
            text_labels = vqa_batch['text_labels'].to(accelerator.device)
            # b n c h w
            pixel_values = vqa_batch['images'].to(accelerator.device).to(weight_type)
            modality_positions = vqa_batch['modality_positions'].to(accelerator.device)
            ...
            if pred_act:
                ...
                loss = config.training.ntp_coeff * loss_ntp + config.training.flow_coeff * loss_flow + config.training.action_coeff * loss_action
            else:
        ...
        if accelerator.sync_gradients:
            ...
            if (global_step + 1) % config.experiment.log_every == 0:
                ...
                Loss_ntp = accelerator.gather(torch.tensor(loss_ntp_m.avg, dtype=weight_type, device=accelerator.device).repeat(total_batch_size_per_gpu)).mean().item()
                if pred_act:
