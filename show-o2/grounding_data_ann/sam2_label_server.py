...
def main():
    ...
    def make_state():
        return {
            ...
            "f8_frame_idx":        0,        # 8/14
            "f9_frame_idx":        0,        # 9/14
            "f10_frame_idx":       0,        # 10/14
            "f11_frame_idx":       0,        # 11/14
            "f12_frame_idx":       0,        # 12/14
            "f13_frame_idx":       0,        # 13/14
            "f14_frame_idx":       0,        # 最终帧
            "active_prompt_key":   "f0",   # f0|...|f14
    ...
    def load_current_file(state):
        ...
        for k in range(15):
            if n <= 1:
                frame_idx_map[f"f{k}_frame_idx"] = 0
            else:
                frame_idx_map[f"f{k}_frame_idx"] = int(round(k * (n - 1) / 14.0))
        state.update({
    ...
    def file_info_text(state):
        ...
        frame_labels = [
            "0", "1/14", "2/14", "3/14", "4/14", "5/14", "6/14", "7/14", "8/14", "9/14",
            "10/14", "11/14", "12/14", "13/14", "最终",
        ]
        frame_text = ", ".join(
            f"{lb}={int(state.get(f'{k}_frame_idx', 0)) + 1}"
            for k, lb in zip(frame_keys, frame_labels)
        )
        return (
            f"📁 文件: {p.name}\n"
            f"📝 指令: {lang}\n"
    ...
    with gr.Blocks(
        ...
        def h_set_frame_mode(state, mode_text):
            ...
            state["active_prompt_key"] = mapping.get(mode_text, "f0")
        ...
        def h_redo_obj(state):
            ...
            for fk in ["f0", "f1", "f2", "f3", "f4", "f5", "f6", "f7", "f8", "f9", "f10", "f11", "f12", "f13", "f14"]:
                fidx = int(state.get(f"{fk}_frame_idx", 0))
